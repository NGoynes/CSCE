...
public class Mario : MonoBehaviour
{
	...
	public static bool jumpSub = false;
    ...
    private bool accessibilityWalk;
    ...
    public bool audioJumped;
    public bool voiceJump = true;
    ...
    void Update()
    {
        if (!inputFreezed)
        {
            ...
            // ** AutoWalk ** - Oliver
            // Also see AutomaticWalk()
            if (Input.GetButtonDown("AutoWalk"))
            {
                // OR operator just toggles the boolean
                accessibilityWalk = accessibilityWalk ^ Input.GetButtonDown("AutoWalk");    //See Edit > Project Settings > Input, in Unity
            }

            if (accessibilityWalk)
            {
                AutomaticWalk(levelEntryWalkSpeedX);
            }
            ...
            // TODO: Needs a better value
            //sensitivity = .5f;
            sensitivity = PlayerPrefs.GetFloat("micVolume");
            ...
            if (level > sensitivity && voiceJump == true/* && !audioJumped*/)
            {
                jumpButtonHeld = true;
                audioJumped = true;
				jumpSub = true;
			}
            else if (level < sensitivity && voiceJump == true/* && audioJumped*/)
            {
                audioJumped = false;
    ...
    {
        // If not using the accessibility walk, then Mario is probably on the flag pole, 
        // and you shouldn't be able to move. - Oliver
        if (!accessibilityWalk)
        {
            FreezeUserInput();
        }
