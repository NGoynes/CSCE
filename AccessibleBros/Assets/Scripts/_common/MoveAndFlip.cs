...
public class MoveAndFlip : MonoBehaviour {
	...
	public bool slowMode = false; // reduces the speed to 1/4th if true
	...
	void FixedUpdate () {
		if (canMove) {
			if(slowMode) {
				m_Rigidbody2D.velocity = new Vector2(Speed.x/4 * directionX, m_Rigidbody2D.velocity.y/4);
			} else {
	...
	void OnCollisionEnter2D(Collision2D other) {
		...
		if (other.gameObject.tag != "Player" && sideHit) {
			...
			if(slowMode) {
				m_Rigidbody2D.velocity = new Vector2(m_Rigidbody2D.velocity.x/4, Speed.y/4);
			} else {
